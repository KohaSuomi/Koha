...
sub send_sms {
    ...
    my $clientid = C4::Context->config('smsProviders')->{'arena'}->{$branchcode}->{'clientid'} || C4::Context->config('smsProviders')->{'arena'}->{'clientid'};
    ...
    my $base_url = C4::Context->config('smsProviders')->{'arena'}->{'sendUrl'};
    my $parameters = {
        'l'         => $self->{_login},
        'p'         => $self->{_password},
