...
sub _rest_call {
    my ($url, $headers, $params) = @_;
    
    my $ua = Mojo::UserAgent->new;
    my $tx = $params ? $ua->post($url => $headers => form => $params) : $ua->post($url => $headers);
    if ($tx->error) {
...
sub send_sms {
    ...
    my $authorization = encode_base64($self->{user}.":".$self->{passwd});
    my $headers = {Authorization => "Bearer $authorization", 'Content-Type' => 'application/x-www-form-urlencoded', 'grant_type' => 'client_credentials'};
    
    my $token = _rest_call($base_url.$appid.'/token', $headers, undef);
    ...
    my $res = _rest_call($base_url.$appid.'/sms', $headers, $params);
