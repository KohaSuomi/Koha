...
Holds.placeHold = function (
    ...
    if (!item || !item.itemnumber) {
        alert("Holds.placeHold():> No Item!");
        return;
    }
    if (!borrower || !borrower.borrowernumber) {
        alert("Holds.placeHold():> No Borrower!");
        return;
    }
    if (!pickupBranch || !pickupBranch.branchcode) {
        alert("Holds.placeHold():> No Pickup Branch!");
        return;
    }
    ...
    $.ajax("/api/v1/holds", {
        ...
        error: function (jqXHR, textStatus, errorThrown) {
            ...
            if (requestBody.itemnumber) {
                //See if this ajax-call was called with an Item.
...
Holds.HoldPicker = function (params) {
    ...
    this._template = function () {
        var html =
            ...
            MSG_CARDNUMBER +
            '" id="hp_cardnumber" type="text" width="16"/>' +
            '  <div id="hp_datepicker"></div>' +
            '  <label for="hp_pickupBranches">' +
            MSG_PICKUP_BRANCH +
            "</label>" +
            Branches.getBranchSelectorHtml({}, "hp_pickupBranches") +
            '  <div class="result"></div>' +
            '  <button id="hp_placeHold">' +
            MSG_PLACE_HOLD +
            '</button><button id="hp_clear">' +
            MSG_CLEAR +
            "</button>" +
            "</fieldset>" +
            "";
    ...
    this.publish = function (publisher, data, event) {
        ...
        if (event == "place_hold_succeeded") {
            $(resultElem).html(
                "<span class='notification' style='color: #00AA00;'>" +
                    MSG_HOLD_PLACED +
                    "</span>"
            );
        } else if (event == "place_hold_failed") {
            $(resultElem).html(
                "<span class='notification' style='color: #AA0000;'>" +
                    data.error +
                    "</span>"
            );
            ...
            alert(
                "Holds.HoldPicker.publish():> Unknown event-type '" +
                    event +
                    "'"
            );
        }
    ...
    this.renderBiblio = function () {
        ...
        if (this.item) {
            html +=
                ...
                " " +
                (this.item.homebranch
                    ? '<strong class="branchcode">' +
                      this.item.homebranch +
                      "</strong>"
                    : "") +
                "</span>";
    ...
    this.renderBorrower = function () {
        ...
        if (!this.borrower) {
            ...
            $(be).html(
                (this.borrower.cardnumber
                    ? this.borrower.cardnumber
                    : this.borrower.borrowernumber) +
                    " " +
                    (this.borrower.surname ? this.borrower.surname : "") +
                    " " +
                    (this.borrower.firstname ? this.borrower.firstname : "")
            );
            this.getCardnumberElement().val(
                this.borrower.cardnumber ? this.borrower.cardnumber : ""
            );
        }
    };
        ...
        this.getCardnumberElement().bind({
            change: function (event) {
                ...
                Borrowers.getBorrowers(
                    { cardnumber: searchTerm, userid: searchTerm },
                    function (jqXHR, textStatus, errorThrown) {
                        if (String(errorThrown.status).search(/^2\d\d/) >= 0) {
                            //Status is OK
                            if (jqXHR[0]) {
                                ...
                                self.publish(
                                    self,
                                    jqXHR[0],
                                    "get_borrower_succeeded"
                                );
                            } else {
                                self.publish(
                                    self,
                                    MSG_NO_SUCH_BORROWER,
                                    "get_borrower_failed"
                                );
